...
int main() {
	...
	string fileparton = "Bp_signal/MR_700_on.lhe.shower"; 
	...
	for(unsigned int ievent=0;ievent<10000;ievent++) { // read and process for each event ===> input the number of events
		unsigned int counter(0), countert(0), counterl(0),countern(0), countertau(0), counterb(0), counterpho(0); // conter to have sure lates
		...
		for(unsigned int ipart=0;ipart<nparticles;ipart++) { // loop on particles
			...
			if(abs(pID) < 6 || pID==21){particles.push_back(fastjet::PseudoJet(Px,Py,Pz,E));particles.at(counter).set_user_index(pID); if(abs(pID) == 5) counterb++; counter++;} 
                        else if (abs(pID)==22) {photons.push_back(fastjet::PseudoJet(Px,Py,Pz,E));if(abs(pID) == 22) counterpho++; }
			...
			else if (abs(pID)==12 || abs(pID)==14 /*|| abs(pID)==16*/) {neutrinos.push_back(fastjet::PseudoJet(Px,Py,Pz,E)); neutrinos.at(countern).set_user_index(pID); countern++;} // close if 
